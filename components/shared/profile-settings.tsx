...
export default function ProfileSettings({
  ...
  const loadAccountActivity = useCallback(() => {
    const now = new Date();
    const lastLoginTime = now.toLocaleTimeString("es-CO", {
      hour: "2-digit",
      minute: "2-digit",
    });
    const lastLoginDate = now.toLocaleDateString("es-CO", {
      day: "2-digit",
      month: "long",
    });
  ...
  useEffect(() => {
    loadUserAvatar();
    loadAccountActivity();
  }, [loadUserAvatar, loadAccountActivity]);
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      if (error) {
        setMessage({
          type: "error",
          text: "Error al reenviar el correo. Intenta de nuevo más tarde.",
        });
        console.error("Supabase resend error:", error);
      } else {
        setMessage({
          type: "success",
          text: "Correo de verificación reenviado. Revisa tu bandeja de entrada (y spam).",
        });
      }
    } catch (e) {
      setMessage({
        type: "error",
        text: "Error inesperado al intentar reenviar el correo.",
      });
            ...
            <CardContent className="space-y-6">
              <div>
                <h4 className="text-md font-semibold mb-2 flex items-center gap-2">
                  <Mail className="h-4 w-4 text-primary" /> Verificación de
                  Correo
                </h4>
                ...
                {isEmailVerified ? (
                  <Alert className="border-green-500 bg-green-50">
                    ...
                    <AlertDescription className="text-green-600">
                      Tu dirección de correo electrónico está confirmada y
                      segura.
                    </AlertDescription>
                  </Alert>
                ) : (
                  <Alert variant="destructive">
                    <AlertTriangle className="h-4 w-4" />
                    <AlertTitle>
                      Acción Requerida: Correo No Verificado
                    </AlertTitle>
                    <AlertDescription>
                      Tu cuenta está activa, pero tu correo electrónico (
                      {user.email}) aún no ha sido confirmado. Haz clic en el
                      botón de abajo para reenviar el enlace de verificación y
                      asegurar tu cuenta.
                    </AlertDescription>
              ...
              <div>
                <h4 className="text-md font-semibold mb-2 flex items-center gap-2">
                  <Activity className="h-4 w-4 text-primary" /> Actividad
                  Reciente
                </h4>
                <Separator className="mb-4" />
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="text-muted-foreground">
                      {accountActivity.lastLogin}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Dispositivo:</span>
                    <span className="text-muted-foreground">
                      {accountActivity.device}
                    </span>
                  </div>
                </div>
              </div>
