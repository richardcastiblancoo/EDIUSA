...
  options?: string[] // Usado solo para multiple_choice
...
  is_active: boolean;
  created_at: string;
  // Campos extra asumidos que existen en tu tabla (para create/update)
...
  try {
    // Extraer solo los campos que existen en la interfaz y potencialmente en la tabla
    const validExamData = {
...
  try {
    // Solo enviar campos que se esperan en la interfaz y que están presentes para la actualización
...
    const { data, error } = await supabase.from("exams").update(updateData).eq("id", id).select().single()
...
  try {
    // Primero obtenemos los IDs de los cursos del profesor
...
    // Eliminar todos los envíos asociados al examen
...
    if (submissionsError) throw submissionsError;
    ...
    // Asumiendo que también tienes una tabla para intentos (exam_attempts)
...
    // Borrar dependencias: envíos, intentos y preguntas (usando la función deleteExam en un loop
    // o usando .in() como está a continuación, que es más eficiente)
    const { error: submissionsError } = await supabase
      .from("exam_submissions")
      .delete()
      .in("exam_id", examIds);
...
// --- FUNCIONES DE ESTUDIANTE Y SUBMISSION ---
...
    // Extraemos los IDs de los cursos
    const courseIds = enrollments.map(enrollment => (enrollment as any).course_id)

    // Obtenemos los exámenes activos y sus calificaciones para estos cursos
    // Nota: La consulta devuelve un tipo más complejo debido a la selección anidada (courses, exam_submissions)
...
    if (examsError) throw examsError
    // La coerción a Exam[] es aproximada debido a los datos anidados, pero es suficiente para la funcionalidad.
...
  exam_id: string;
  student_id: string;
  answers: Record<number, string>; // Asumiendo que las respuestas se guardan como JSON
  time_spent: number;
  warnings: string[];
